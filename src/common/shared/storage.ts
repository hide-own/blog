...
interface StoreItemDetails<K extends keyof D, D extends Record<string, unknown>> {
	namespace: string
	index: number
	expire: number | null
	key: K
...
function isStoreItem<D>(v: unknown): v is StoreItem<D> {
	return v != null && typeof v === "object" && !Array.isArray(v) && hasOwn(v, "value") && (!hasOwn(v, "expire") || typeof (v as { expire: unknown }).expire === "number" || (v as { expire: unknown }).expire == null)
}
...
export class StorageProxy<Key extends keyof Data, Data extends Record<string, unknown>> {
