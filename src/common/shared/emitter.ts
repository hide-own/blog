...
export function listen<Key extends GlobalKey>(type: Key, onData: ValueChanged<GlobalData[Key]>): VoidCallback {
	if (!(type in events)) {
...
export function emit<Key extends GlobalKey>(type: Key, data: GlobalData[Key]): void {
	events[type]?.emit(data)
}

export function wait<Key extends GlobalKey>(type: Key, duration?: number): Promise<GlobalData[Key]> {
	if (!(type in events)) {
		events[type] = createChangeEmitter()
	}
