...
async function handleAxiosError<T>(axiosError: AxiosError<Record<string, unknown>>): Promise<ServiceRequestResult<T>> {
	...
	// 请求不成功的错误
	const errorCode: ErrorStatus = ((axiosError.response as AxiosResponse | undefined)?.status as ErrorStatus) || "DEFAULT"
 ...
 * @return {ServiceRequestResult}
 */
async function handleResponseError<T = unknown>(axios: AxiosInstance, response: AxiosResponse): Promise<ServiceRequestResult<T>> {
	const error: ServiceRequestError = {
		type: "http",
		code: DEFAULT_REQUEST_ERROR_CODE,
		msg: DEFAULT_REQUEST_ERROR_MSG
...
export function createAxios(axiosConfig: AxiosRequestConfig): AxiosInstance {
	...
	instance.interceptors.response.use(
		(response: AxiosResponse): Promise<ServiceRequestResult> | ServiceRequestResult => {
			// HTTP响应状态不在有效氛围内。
...
export interface RequestUtils {
	 ...
	 */
	get<T = unknown>(url: string, config?: AxiosRequestConfig): Promise<ServiceRequestResult<T>>
	 ...
	 * @param config - axios配置
	 */
	post<T = unknown>(url: string, data?: unknown, config?: AxiosRequestConfig): Promise<ServiceRequestResult<T>>
	 ...
	 * @param config - axios配置
	 */
	put<T = unknown>(url: string, data?: unknown, config?: AxiosRequestConfig): Promise<ServiceRequestResult<T>>
...
export function createRequestUtils(axiosConfig: AxiosRequestConfig): RequestUtils {
	...
	return {
		get: <T = unknown>(url: string, config?: AxiosRequestConfig): Promise<ServiceRequestResult<T>> => getRequestResponse(instance, "get", url, undefined, config),
		post: <T = unknown>(url: string, data?: unknown, config?: AxiosRequestConfig): Promise<ServiceRequestResult<T>> => getRequestResponse(instance, "post", url, data, config),
		put: <T = unknown>(url: string, data?: unknown, config?: AxiosRequestConfig): Promise<ServiceRequestResult<T>> => getRequestResponse(instance, "put", url, data, config),
		delete: <T = unknown>(url: string, config: AxiosRequestConfig): Promise<ServiceRequestResult<T>> => getRequestResponse(instance, "delete", url, undefined, config)
	}
}

async function getRequestResponse<T>(instance: AxiosInstance, method: RequestMethod, url: string, data?: unknown, config?: AxiosRequestConfig): Promise<ServiceRequestResult<T>> {
	switch (method) {
		case "get":
